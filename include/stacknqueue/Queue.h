...
public:
    ...
    T pop()
    {
        ...
        T item = list.get(0); // Lấy phần tử đầu tiên
        list.removeAt(0);     // Xóa phần tử đầu tiên
    ...
    T &peek()
    {
        ...
        return list.get(0); // Trả về tham chiếu phần tử đầu tiên
    }
    bool empty()
    {
    ...
    bool remove(T item)
    {
        ...
        {
            if (equals(list.get(i), item, itemEqual))
            {
                list.removeAt(i); // Xóa phần tử
                return true;
            }
        }
        return false;
